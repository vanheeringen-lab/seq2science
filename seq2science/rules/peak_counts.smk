...
def get_coverage_table_replicates(file_ext):
    def wrapped(wildcards):
        if wildcards.peak_caller == "macs2":
            return expand([f"{{final_bam_dir}}/{wildcards.assembly}-{replicate}.samtools-coordinate.{file_ext}"
                for replicate in treps[treps['assembly'] == wildcards.assembly].index], **config)
        elif wildcards.peak_caller == "genrich":
            return expand([f"{{final_bam_dir}}/{wildcards.assembly}-{replicate}.sambamba-queryname.{file_ext}"
                for replicate in treps[treps['assembly'] == wildcards.assembly].index], **config)
        else:
