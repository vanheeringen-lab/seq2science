...
#### Count table
A useful result the pipeline outputs is the count table, located at {count_table}/{peak_caller}/count_table_{assembly}.bed. This count table is generated by [gimmemotifs](https://gimmemotifs.readthedocs.io/en/master/)' coverage table command. By default ... 

TODO: combine_peaks -> coverage_table
 ```
				sample1		sample2
...
```
**Note:** 
The count table uses the number of raw reads under each peak, and does not use any kind of correction for the difference in total number of reads between samples!
